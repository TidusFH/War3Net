    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                // Auto-initialize extended TriggerData for YDWE/KKWE/dzapi support
                // This will auto-generate if not found
                ExtendedTriggerData.AutoInitialize();
                Console.WriteLine();

                // Check for special commands first
                if (args.Length > 0 && (args[0] == "--generate-triggerdata" || args[0] == "-gtd"))
                {
                    GenerateExtendedTriggerData();
                    return;
                }
                ...
                else
                {
                    ...
                    if (!Directory.Exists("../Source"))
                    {
                        ...
                        Console.WriteLine("Created ../Source folder - place your source .w3x or .wtg files there");
                    }
                    if (!Directory.Exists("../Target"))
                    {
                        Directory.CreateDirectory("../Target");
                        Console.WriteLine("Created ../Target folder - place your target .w3x or .wtg files there");
                    }
                ...
                while (true)
                {
                    ...
                    Console.WriteLine($"g. GENERATE: Generate ExtendedTriggerData.txt for YDWE/KKWE support {(ExtendedTriggerData.IsExtended ? "(loaded)" : "")}");
                    ...
                    switch (choice)
                    {
                        ...
                        case "g":
                        case "G":
                            Console.WriteLine("\n=== Generate Extended TriggerData ===");
                            Console.WriteLine("This will create ExtendedTriggerData.txt with YDWE/KKWE/dzapi support.\n");
                            
                            // Find paths
                            var war3PatchesDir = FindWar3PatchesFolder();
                            var baseTriggerDataPath = FindBaseTriggerData();
                            var outputTriggerData = "ExtendedTriggerData.txt";
                            
                            if (war3PatchesDir == null)
                            {
                                Console.ForegroundColor = ConsoleColor.Red;
                                Console.WriteLine("ERROR: Could not find 'War3 Patches' folder.");
                                Console.WriteLine("Please ensure it exists relative to this executable.");
                                Console.ResetColor();
                                break;
                            }
                            
                            if (baseTriggerDataPath == null)
                            {
                                Console.ForegroundColor = ConsoleColor.Red;
                                Console.WriteLine("ERROR: Could not find base TriggerData.txt.");
                                Console.WriteLine("Please ensure War3Net source is available.");
                                Console.ResetColor();
                                break;
                            }
                            
                            Console.WriteLine($"War3 Patches folder: {war3PatchesDir}");
                            Console.WriteLine($"Base TriggerData: {baseTriggerDataPath}");
                            Console.WriteLine($"Output: {outputTriggerData}");
                            Console.WriteLine();
                            
                            if (ExtendedTriggerData.GenerateAndInitialize(war3PatchesDir, baseTriggerDataPath, outputTriggerData))
                            {
                                Console.ForegroundColor = ConsoleColor.Green;
                                Console.WriteLine("\n✓ Extended TriggerData generated and loaded!");
                                Console.WriteLine("  YDWE/KKWE maps should now parse correctly.");
                                Console.ResetColor();
                            }
                            break;
        ...
        static MapTriggers ReadWTGFile(string filePath)
        {
            ...
            try
            {
                // Use ExtendedTriggerData if available (includes YDWE/KKWE/dzapi functions)
                var triggerData = ExtendedTriggerData.Instance;
                var triggers = (MapTriggers)constructorInfo.Invoke(new object[] { reader, triggerData });
                return triggers;
            }
            catch (System.Reflection.TargetInvocationException ex)
            {
                ...
                if (innerMessage.Contains("was not present in the dictionary"))
                {
                    throw new InvalidOperationException(
                        $"Failed to parse war3map.wtg file. " +
                        $"Inner error: {innerMessage}\n\n" +
                        $"This error usually means the WTG file was created with YDWE, KKWE, or another\n" +
                        $"modded World Editor that uses custom trigger functions not in the default TriggerData.\n\n" +
                        $"To fix this:\n" +
                        $"  1. Run: generate-triggerdata (option 'g' in menu)\n" +
                        $"  2. This creates ExtendedTriggerData.txt with YDWE/KKWE functions\n" +
        ...
        /// CRITICAL: Must build ID mapping BEFORE changing any IDs, otherwise ParentId matching fails
        /// </summary>
        static void FixDuplicateIds(MapTriggers triggers)
        {
            ...
            Console.WriteLine("  This is expected for 1.27 format (trigger IDs not stored in file)");
            Console.WriteLine("  Reassigning unique IDs while preserving ParentId relationships...");
            Console.ResetColor();

            // CRITICAL: Build old ID -> new ID mapping BEFORE changing any IDs!
            var oldIdToNewId = new Dictionary<int, int>();
            for (int i = 0; i < triggers.TriggerItems.Count; i++)
            {
                int oldId = triggers.TriggerItems[i].Id;
                int newId = i; // Sequential: 0, 1, 2, 3...

                // Add to mapping (may have multiple items with same old ID)
                ...
                {
                    Console.WriteLine($"[DEBUG] Will change '{triggers.TriggerItems[i].Name}' ID: {oldId} -> {newId}");
                }
            }

            // Now reassign IDs to ALL items
            for (int i = 0; i < triggers.TriggerItems.Count; i++)
            {
            ...
            // Do NOT update trigger ParentIds - they already point to the correct category IDs!
            // For 1.27 format:
            //   - Category IDs are READ from file (may be non-sequential: 0, 1, 2, 4, 3, 6, 8, 25...)
            //   - Trigger ParentIds are READ from file (pointing to those category IDs)
            //   - We must preserve this relationship!
            // The only thing we need to do is normalize root-level ParentIds

            foreach (var item in triggers.TriggerItems)
            {
                // Normalize root-level items to appropriate value for format
            ...
            if (DEBUG_MODE)
            {
                Console.WriteLine($"[DEBUG] Reassigned {triggers.TriggerItems.Count} item IDs, ParentIds preserved");
            }

            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine($"  ✓ Reassigned IDs: 0 to {triggers.TriggerItems.Count - 1}");
            Console.WriteLine($"  ✓ Preserved ParentId relationships from original file");
            Console.ResetColor();
        }
        ...
        static string AutoDetectMapFile(string folderPath)
        {
            ...
            throw new FileNotFoundException($"No map files found in {Path.GetFullPath(folderPath)}.\n" +
                $"Please place your .w3x, .w3m, or war3map.wtg files in this folder.\n" +
                $"Or run with command line arguments: WTGMerger.exe <source> <target> [output]");
        }

        /// <summary>
        /// Generates ExtendedTriggerData.txt from War3 Patches folder
        /// </summary>
        static void GenerateExtendedTriggerData()
        {
            Console.WriteLine("=== Generating Extended TriggerData for YDWE/KKWE/dzapi support ===\n");

            var war3PatchesDir = FindWar3PatchesFolder();
            var baseTriggerDataPath = FindBaseTriggerData();
            var outputTriggerData = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ExtendedTriggerData.txt");
            ...
            if (war3PatchesDir == null)
            {
                ...
                Console.WriteLine("  - War3 Patches");
                Console.WriteLine("  - ../War3 Patches");
                Console.WriteLine("  - ../../War3 Patches");
                Console.ResetColor();
                Environment.Exit(1);
            ...
            if (baseTriggerDataPath == null)
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("ERROR: Could not find base TriggerData.txt.");
                Console.WriteLine("Please ensure War3Net source is available at:");
                Console.WriteLine("  - src/War3Net.Build.Core/Resources/TriggerData.txt");
                Console.ResetColor();
                Environment.Exit(1);
            }
            ...
            try
            {
                TriggerDataMerger.MergeTriggerData(war3PatchesDir, baseTriggerDataPath, outputTriggerData);

                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine("\n✓ Extended TriggerData generated successfully!");
                Console.WriteLine($"  Output: {outputTriggerData}");
                Console.WriteLine("\n  This file will be auto-loaded when WTGMerger starts.");
                Console.WriteLine("  YDWE/KKWE/dzapi maps should now parse correctly.");
                Console.ResetColor();
            }
            catch (Exception ex)
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine($"\n❌ Error generating TriggerData: {ex.Message}");
                Console.ResetColor();
                Environment.Exit(1);
            }
        }
        ...
        /// <summary>
        /// Finds the War3 Patches folder containing YDWE/KKWE/dzapi definitions
        /// </summary>
        static string? FindWar3PatchesFolder()
        {
            var baseDir = AppDomain.CurrentDomain.BaseDirectory;
            var searchPaths = new[]
            {
                // First check in executable directory (copied during build)
        ...
        /// <summary>
        /// Finds the base TriggerData.txt from War3Net source
        /// </summary>
        static string? FindBaseTriggerData()
        {
            var baseDir = AppDomain.CurrentDomain.BaseDirectory;
            var searchPaths = new[]
            {
                ...
                "../../src/War3Net.Build.Core/Resources/TriggerData.txt",
                "../../../src/War3Net.Build.Core/Resources/TriggerData.txt",
                Path.Combine(baseDir, "..", "src/War3Net.Build.Core/Resources/TriggerData.txt"),
                Path.Combine(baseDir, "..", "..", "src/War3Net.Build.Core/Resources/TriggerData.txt"),
            };
            ...
            catch (System.Reflection.TargetInvocationException ex)
            {
                ...
                if (innerMessage.Contains("was not present in the dictionary"))
                {
                    throw new InvalidOperationException(
                        ...
                        $"Inner error: {innerMessage}\n\n" +
                        $"This error usually means the map was created with YDWE, KKWE, or another\n" +
                        $"modded World Editor that uses custom trigger functions not in the default TriggerData.\n\n" +
                        $"To fix this:\n" +
                        $"  1. Run: generate-triggerdata (option 'g' in menu)\n" +
                        $"  2. This creates ExtendedTriggerData.txt with YDWE/KKWE functions\n" +
